...
#ifndef ACF_INCLUDE_INTERNAL_ACF_STRING_WRAPPERS_H_
#define ACF_INCLUDE_INTERNAL_ACF_STRING_WRAPPERS_H_
#pragma once
...
///
struct AcfStringTraitsUTF8 {
  typedef char char_type;
  typedef acf_string_utf8_t struct_type;
  typedef acf_string_userfree_utf8_t userfree_struct_type;

  static inline void clear(struct_type* s) { acf_string_utf8_clear(s); }
  static inline int set(const char_type* src,
                        size_t src_size,
                        struct_type* output,
                        int copy) {
    return acf_string_utf8_set(src, src_size, output, copy);
  }
  static inline int compare(const struct_type* s1, const struct_type* s2) {
    return acf_string_utf8_cmp(s1, s2);
  }
  static inline userfree_struct_type userfree_alloc() {
    return acf_string_userfree_utf8_alloc();
  }
  static inline void userfree_free(userfree_struct_type ufs) {
    return acf_string_userfree_utf8_free(ufs);
  }
  ...
  static inline bool from_string(const std::string::value_type* data,
                                 size_t length,
                                 struct_type* s) {
    return acf_string_utf8_copy(data, length, s) ? true : false;
  }
  ...
  static inline std::u16string to_string16(const struct_type* s) {
    acf_string_utf16_t cstr;
    memset(&cstr, 0, sizeof(cstr));
    acf_string_utf8_to_utf16(s->str, s->length, &cstr);
    std::u16string str;
    if (cstr.length > 0) {
      str = std::u16string(
          reinterpret_cast<std::u16string::value_type*>(cstr.str), cstr.length);
    }
    acf_string_utf16_clear(&cstr);
    return str;
  }
  static inline bool from_string16(const std::u16string::value_type* data,
                                   size_t length,
                                   struct_type* s) {
    return acf_string_utf16_to_utf8(reinterpret_cast<const char16*>(data),
                                    length, s)
               ? true
               : false;
  }
...
///
struct AcfStringTraitsUTF16 {
  typedef char16 char_type;
  typedef acf_string_utf16_t struct_type;
  typedef acf_string_userfree_utf16_t userfree_struct_type;

  static inline void clear(struct_type* s) { acf_string_utf16_clear(s); }
  static inline int set(const char_type* src,
                        size_t src_size,
                        struct_type* output,
                        int copy) {
    return acf_string_utf16_set(src, src_size, output, copy);
  }
  static inline int compare(const struct_type* s1, const struct_type* s2) {
    return acf_string_utf16_cmp(s1, s2);
  }
  static inline userfree_struct_type userfree_alloc() {
    return acf_string_userfree_utf16_alloc();
  }
  static inline void userfree_free(userfree_struct_type ufs) {
    return acf_string_userfree_utf16_free(ufs);
  }

  // Conversion methods.
  static inline std::string to_string(const struct_type* s) {
    acf_string_utf8_t cstr;
    memset(&cstr, 0, sizeof(cstr));
    acf_string_utf16_to_utf8(s->str, s->length, &cstr);
    std::string str;
    if (cstr.length > 0) {
      str = std::string(cstr.str, cstr.length);
    }
    acf_string_utf8_clear(&cstr);
    return str;
  }
  static inline bool from_string(const std::string::value_type* data,
                                 size_t length,
                                 struct_type* s) {
    return acf_string_utf8_to_utf16(data, length, s) ? true : false;
  }
  static inline bool from_string(const std::string& str, struct_type* s) {
  ...
  static inline bool from_string16(const std::u16string::value_type* data,
                                   size_t length,
                                   struct_type* s) {
    return acf_string_utf16_set(reinterpret_cast<const char16*>(data), length,
                                s, true)
               ? true
               : false;
  }
...
class AcfStringBase {
 public:
  ...
  /// Default constructor.
  ///
  AcfStringBase() : string_(NULL), owner_(false) {}

  ///
  /// Create a new string from an existing string. Data will always be copied.
  ///
  AcfStringBase(const AcfStringBase& str) : string_(NULL), owner_(false) {
  ...
  /// type.
  ///
  AcfStringBase(const std::string& src) : string_(NULL), owner_(false) {
    FromString(src);
  }
  AcfStringBase(const char* src, size_t length = 0)
  ...
  /// type.
  ///
  AcfStringBase(const std::u16string& src) : string_(NULL), owner_(false) {
    FromString16(src);
  }
  AcfStringBase(const std::u16string::value_type* src, size_t length = 0)
      : string_(NULL), owner_(false) {
    if (src) {
  ...
  AcfStringBase(const char16* src, size_t length = 0)
      : string_(NULL), owner_(false) {
    if (src) {
  ...
  ///
  AcfStringBase(const char_type* src, size_t src_len, bool copy)
      : string_(NULL), owner_(false) {
    if (src && src_len > 0) {
  ...
  ///
  AcfStringBase(const struct_type* src) : string_(NULL), owner_(false) {
    if (!src) {
      return;
    }
  ...
  virtual ~AcfStringBase() { ClearAndFree(); }
  ...
  ///
  int compare(const AcfStringBase& str) const {
    if (empty() && str.empty()) {
      return 0;
    }
    if (empty()) {
  ...
  ///
  bool operator<(const AcfStringBase& str) const { return (compare(str) < 0); }
  bool operator<=(const AcfStringBase& str) const {
  ...
  ///
  AcfStringBase& operator=(const AcfStringBase& str) {
    FromString(str.c_str(), str.length(), true);
    return *this;
  }
  operator std::string() const { return ToString(); }
  AcfStringBase& operator=(const std::string& str) {
    FromString(str);
    return *this;
  }
  AcfStringBase& operator=(const std::string::value_type* str) {
    FromString(str);
    return *this;
  }
  operator std::u16string() const { return ToString16(); }
  AcfStringBase& operator=(const std::u16string& str) {
    FromString16(str);
    return *this;
  }
  AcfStringBase& operator=(const std::u16string::value_type* str) {
    FromString16(str);
    return *this;
  }
...
typedef AcfStringBase<AcfStringTraitsUTF8> AcfStringUTF8;
typedef AcfStringBase<AcfStringTraitsUTF16> AcfStringUTF16;

#endif  //! ACF_INCLUDE_INTERNAL_ACF_STRING_WRAPPERS_H_
